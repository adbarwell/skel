...
-spec loop(non_neg_integer(), dict:dict(), {pid(), reference()}) -> 'eos'.
%% @doc Recursively receives and stores messages until they are ready for
%% release.
loop(Seen, Dict, {NextPid, _} = NextPRef) ->
  receive
    {data, _, _} = DataMessage ->
      {{ord, Idx}, DataMessage1} = sk_data:pop(DataMessage),
      Dict1 = store(Idx, DataMessage1, Dict),
      {Seen1, Dict2} = forward(Seen, Dict1, NextPRef),
      loop(Seen1, Dict2, NextPRef);
    {system, eos} ->
...
-spec forward(non_neg_integer(), dict:dict(), {pid(), reference()}) -> {non_neg_integer(), dict:dict()}.
...
forward(Seen, Dict, {NextPid, _} = NextPRef) ->

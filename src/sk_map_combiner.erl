...
start(NextPRef) ->
  ...
  loop(auto, 0, 0, dict:new(), recomp_with(), NextPRef).


-spec start(pref(), pos_integer()) -> 'eos'.
%% @doc Initialises the recomposition process for when the number of workers
...
start(NextPRef, NWorkers) ->
  sk_tracer:t(75, self(), {?MODULE, start}, [{next_pid, NextPRef}]),
  loop(man, NWorkers, 0, dict:new(), recomp_with(), NextPRef).


-spec loop(atom(), non_neg_integer(), non_neg_integer(), dict:dict(), data_recomp_fun(), pref()) -> 'eos'.
...
loop(auto, TotWorkers, DeadWorkers, Dict, DataCombinerFun, {NextPid, _} = NextPRef) ->
  receive
    {data, _, _} = PartitionMessage ->
      {{decomp, Ref, Idx, NPartitions}, PartitionMessage1} =
              sk_data:pop(PartitionMessage),
      Dict1 = store(Ref, Idx, NPartitions, PartitionMessage1, Dict),
      Dict2 = combine_and_forward(Ref, Dict1, DataCombinerFun, NextPRef),
      TotWorkers1 = new_total_workers(TotWorkers, NPartitions),
      loop(auto, TotWorkers1, DeadWorkers, Dict2, DataCombinerFun, NextPRef);
    {system, eos} when DeadWorkers+1 >= TotWorkers ->
      sk_tracer:t(75, self(), NextPRef, {?MODULE, system},
                  [{msg, eos}, {total, TotWorkers}, {dead, DeadWorkers+1}]),
      NextPid ! {system, eos},
      eos;
    {system, eos} ->
      sk_tracer:t(85, self(), {?MODULE, system}, [{msg, eos}, {total, TotWorkers}, {dead, DeadWorkers+1}]),
      loop(auto, TotWorkers, DeadWorkers+1, Dict, DataCombinerFun, NextPRef)
  end;
loop(man, TotWorkers, DeadWorkers, Dict, DataCombinerFun, {NextPid, _} = NextPRef) ->
  receive
    {data, _, _} = PartitionMessage ->
      {{decomp, Ref, Idx, NPartitions}, PartitionMessage1} = sk_data:pop(PartitionMessage),
      Dict1 = store(Ref, Idx, NPartitions, PartitionMessage1, Dict),
      Dict2 = combine_and_forward(Ref, Dict1, DataCombinerFun, NextPRef),
      loop(man, TotWorkers, DeadWorkers, Dict2, DataCombinerFun, NextPRef);
    {system, eos} when DeadWorkers+1 >= TotWorkers ->
      sk_tracer:t(75, self(), NextPRef, {?MODULE, system}, [{msg, eos}, {total, TotWorkers}, {dead, DeadWorkers+1}]),
      NextPid ! {system, eos},
      eos;
    {system, eos} ->
      sk_tracer:t(85, self(), {?MODULE, system}, [{msg, eos}, {total, TotWorkers}, {dead, DeadWorkers+1}]),
      loop(man, TotWorkers, DeadWorkers+1, Dict, DataCombinerFun, NextPRef)
  end.
...
-spec combine_and_forward(reference(), dict:dict(), data_recomp_fun(), pref()) -> dict:dict().
...
%% `Dict'.
combine_and_forward(Ref, Dict, DataCombinerFun, NextPRef) ->
  case dict:find({Ref, expecting}, Dict) of
      error ->
          Dict;
      {ok, NPartitions} ->
          combine_and_forward(Ref, Dict, NPartitions, DataCombinerFun, NextPRef)
  end.


-spec combine_and_forward(reference(),
                          dict:dict(),
                          pos_integer(),
                          data_recomp_fun(),
                          pref()) -> dict:dict().
...
combine_and_forward(Ref, Dict,
                    NPartitions, DataCombinerFun, {NextPid, _} = NextPRef) ->
    ...
    if
        RcvdPartitions == NPartitions ->
            ...
            sk_tracer:t(50, self(),
                        NextPRef, {?MODULE, data},
                        [{ref, Ref}, {output, DataMessage},

...
-spec start(recomp_fun(), pref()) -> 'eos'.
...
start(Recomp, NextPRef) ->
  sk_tracer:t(75, self(), {?MODULE, start}, [{next_pid, NextPRef}]),
  DataRecompFun = sk_data:recomp_with(Recomp),
  loop(dict:new(), DataRecompFun, NextPRef).

-spec loop(dict:dict(), data_recomp_fun(), pref()) -> 'eos'.
...
loop(Dict, DataRecompFun, {NextPid, _} = NextPRef) ->
  receive
    {data, _, _} = PartitionMessage ->
      ...
      Dict2 = combine_and_forward(Ref, Dict1, DataRecompFun, NextPRef),
      loop(Dict2, DataRecompFun, NextPRef);
    {system, eos} ->
...
combine_and_forward(Ref, Dict, DataCombinerFun, NextPRef) ->
  case dict:find({Ref, expecting}, Dict) of
    error             -> Dict;
    {ok, NPartitions} -> combine_and_forward(Ref, Dict, NPartitions, DataCombinerFun, NextPRef)
  end.

-spec combine_and_forward(reference(), dict:dict(), pos_integer(), data_recomp_fun(), pref()) -> dict:dict().
...
%% is returned.
combine_and_forward(Ref, Dict, NPartitions, DataCombinerFun, {NextPid, _} = NextPRef) ->
    ...
    if
        RcvdPartitions == NPartitions ->
            ...
            sk_tracer:t(50, self(), NextPRef, {?MODULE, data}, [{ref, Ref}, {output, DataMessage}, {partitions, PartitionMessages}]),
            NextPid ! DataMessage,
